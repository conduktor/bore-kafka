    ...
    S2: AsyncRead + AsyncWrite + Unpin,
{
    ...
    tokio::select! {
        ...
        res = local_to_remote(local_read, remote_write, codec,tx_metadata,rx_mapping) => res,
    }
}
...
pub async fn adapt_metadata(
    ...
    tx_metadata.send(new_brokers).await;
    ...
    source
        .then(|item| async { match item {
            Ok(KafkaResponse::Metadata(version, header, response)) => Ok(KafkaResponse::Metadata(
                version,
                header,
                adapt_metadata(response, tx_metadata.clone(), rx_mapping.clone()).await,
            )),
            other => other,
        }})
        .forward(sink)
